...
| GET | `/admin/backtest/compare` | Access | 一括投入 vs 段階エントリー (entry 軸) / 全量利確 vs 部分利確+ATRトレーリング (exit 軸) の比較 backtest (同一 bars/rule/コスト、実発注なし)。`variants` は `<entry>[+<exit>]` 形式 (`+<exit>` 省略時は `preset`) |
