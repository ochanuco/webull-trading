...
| `cash_fallback_sell_mode` | `'off'` | 退避先銘柄の需要連動自動 SELL (#452 follow-up)。`off` / `observe` (計画を log に残すだけ) / `enforce`。退避元が同 cron で BUY を試みた時だけ (保有だけでは需要にならない)、退避先の active weight 超過分 (band 10% 超) を部分 SELL して本体戦略に資金を戻す |
